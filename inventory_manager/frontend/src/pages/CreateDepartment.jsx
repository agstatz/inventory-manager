...
import { Center, Heading, Box, Stack, Button,
         FormControl, FormLabel, FormErrorMessage, 
         Alert, AlertIcon, HStack, Input } from '@chakra-ui/react';
...
export default class CreateDepartment extends Component {
    ...
    handleSubmit() {
        ...
        fetch('/api/department', requestOptions).then((response) =>
            response.json()
        ).then((data) => {
            console.log(data);
                    ...
                    <Center>
                        <Box bg="brand.500" borderRadius="5px" maxWidth="1020px" m={[10, 10, 30, 40]} mt={[20, 20, 20, 20]} p={1}>
                            ...
                            <FormControl isInvalid={this.state.id_err}>
                                <FormLabel htmlFor="department_id">Department ID</FormLabel>
                                <Input 
                                    id="department_id"
                                    placeholder='MKTNG'
                                    variant='outline'
                                ...
                                <FormErrorMessage>Department ID is required.</FormErrorMessage>

                            </FormControl>
                            <FormControl isInvalid={this.state.name_err}>
                                <FormLabel htmlFor="department_id">Department Name</FormLabel>
                                <Input 
                                    id="department_id"
                            ...
                            <HStack spacing={2} mt={2}>
                                ...
                                <Link to="/department/"><Button type="cancel">
                                    Cancel
                                </Button></Link>
                            </HStack>
                            {this.state.success ? (<Alert status='success' borderRadius="5px" variant="subtle">
                                    <AlertIcon />
                                    Department created successfully.
                                </Alert>) : (<></>)}
                            {this.state.failure.length > 0 ? (<Alert status='error' borderRadius="5px" variant="subtle">
                                <AlertIcon />
                                {this.state.failure}
                            </Alert>) : (<></>)}
                            </Stack>
